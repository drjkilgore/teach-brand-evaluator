// #TEACH Brand Evaluator — Supabase Edge Function (v4.1)
// Deployed from GitHub via Actions (or pasted in the dashboard editor).
// Secret required: ANTHROPIC_API_KEY (Edge Functions -> Secrets).
...
Deno.serve(async (req) => {
  ...
  // Self-test: GET returns version, env state, and a live 1-token Anthropic ping.
  if (req.method === "GET") {
    const keySet = !!Deno.env.get("ANTHROPIC_API_KEY");
    let ping = "skipped (ANTHROPIC_API_KEY secret not set)";
    if (keySet) {
      try {
        ...
        ping = r.ok ? "OK — Anthropic reachable, key valid, credits present"
          : "FAILED (" + r.status + "): " + ((body && body.error && body.error.message) || JSON.stringify(body).slice(0, 160));
      } catch (e) {
        ping = "FAILED: " + String((e && e.message) || e);
      }
    }
    return json({ version: "v4.1", anthropic_key_set: keySet, anthropic_ping: ping }, 200);
  }
  ...
  catch (_e) { return json({ error: "Bad JSON body" }, 400); }

  const { data, mediaType, pieceType, isTemplate, context, fileName } = payload || {};
  if (!data || !mediaType) return json({ error: "Missing file data" }, 400);

  const apiKey = Deno.env.get("ANTHROPIC_API_KEY");
  if (!apiKey) return json({ error: "ANTHROPIC_API_KEY secret is not set in Supabase (Edge Functions -> Secrets)" }, 500);
  ...
  let parsed;
  try {
    console.log("[evaluate] calling Anthropic, media=" + mediaType + ", b64_len=" + data.length);
    const t0 = Date.now();
    ...
    console.log("[evaluate] Anthropic responded " + resp.status + " in " + Math.round((Date.now() - t0) / 1000) + "s");
    if (!resp.ok) return json({ error: (out && out.error && out.error.message) || ("Anthropic API error (status " + resp.status + ")") }, 502);

    let text = "";
    for (const c of (out.content || [])) if (c.type === "text") text += c.text;
    text = text.replace(/```json|```/g, "").trim();
    try { parsed = JSON.parse(text); }
    catch (_e) {
      const m = text.match(/\{[\s\S]*\}/);
      if (m) parsed = JSON.parse(m[0]);
      else return json({ error: "Model returned non-JSON output" }, 502);
    }
  } catch (err) {
    const msg = (err && err.name === "TimeoutError")
      ? "Anthropic did not respond within 90 seconds — try again; if it persists, check status.anthropic.com"
      : String((err && err.message) || err);
    console.log("[evaluate] ERROR: " + msg);
    return json({ error: msg }, 500);
  }

  // Best-effort history write — detached; failures ignored; works fine with no table.
  const writeHistory = async () => {
    const url = Deno.env.get("SUPABASE_URL");
    const key = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");
    if (!url || !key) return;
    await fetch(url.replace(/\/$/, "") + "/rest/v1/evaluations", {
      method: "POST",
      headers: {
        "apikey": key,
      ...
      body: JSON.stringify({
        status: "done",
        piece_name: fileName || null,
        piece_type: pieceType || null,
        brand_score: parsed.brand_score,
  ...
  try {
    ...
    if (rt && rt.waitUntil) rt.waitUntil(writeHistory().catch(() => {}));
    else writeHistory().catch(() => {});
  } catch (_e) { /* history is optional */ }
