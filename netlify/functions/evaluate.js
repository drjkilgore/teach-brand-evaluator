...
// #TEACH Brand Evaluator — serverless proxy to the Anthropic API.
...
exports.handler = async function (event) {
  ...
  if (!data || !mediaType) return { statusCode: 400, headers, body: JSON.stringify({ error: "Missing file data" }) };
  if (!process.env.ANTHROPIC_API_KEY) return { statusCode: 500, headers, body: JSON.stringify({ error: "ANTHROPIC_API_KEY not configured in Netlify environment variables" }) };
  ...
  try {
    ...
    if (!resp.ok) {
      return { statusCode: 502, headers, body: JSON.stringify({ error: (dataOut && dataOut.error && dataOut.error.message) || "Anthropic API error" }) };
    }

    let text = "";
    (dataOut.content || []).forEach(function (c) { if (c.type === "text") text += c.text; });
    text = text.replace(/```json|```/g, "").trim();

    let parsed;
    try { parsed = JSON.parse(text); }
    catch (e) {
      // salvage: grab the outermost JSON object
      const m = text.match(/\{[\s\S]*\}/);
      if (m) { parsed = JSON.parse(m[0]); }
      else { return { statusCode: 502, headers, body: JSON.stringify({ error: "Model returned non-JSON", raw: text.slice(0, 400) }) }; }
    }
